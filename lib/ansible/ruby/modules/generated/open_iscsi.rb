  ...
  module Ruby
    module Modules
      class Open_iscsi < Base
        ...
        # @return [String] whether the target node should be connected
        attribute :login
        ...
        # @return [Object] whether the target node should be automatically connected at startup
        ...
        # @return [String] whether the list of target nodes on the portal should be (re)discovered and added to the persistent iscsi database. Keep in mind that iscsiadm discovery resets configurtion, like node.startup to manual, hence combined with auto_node_startup=yes will allways return a changed state.
        attribute :discover
        validates :discover, inclusion: {:in=>[true, false], :message=>"%{value} needs to be true, false"}, allow_nil: true

        # @return [String] whether the list of nodes in the persistent iscsi database should be returned by the module
