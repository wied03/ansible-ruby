  ...
  module Ruby
    module Modules
      class Rds < Base
        ...
        # @return [String] Database instance identifier. Required except when using command=facts or command=delete on just a snapshot
        attribute :instance_name
        validates :instance_name, type: String

        # @return [String] Name of the database to replicate. Used only when command=replicate.
        ...
        # @return [String] The instance type of the database.  Must be specified when command=create. Optional when command=replicate, command=modify or command=restore. If not specified then the replica inherits the same instance type as the source instance.
        attribute :instance_type
        validates :instance_type, type: String

        # @return [String] Master database username. Used only when command=create.
        attribute :username
        validates :username, type: String

        # @return [String] Password for the master database username. Used only when command=create or command=modify.
        attribute :password
        ...
        # @return [Object] Name of a database to create within the instance.  If not specified then no database is created. Used only when command=create.
        attribute :db_name

        # @return [Object] Version number of the database engine to use. Used only when command=create. If not specified then the current Amazon RDS default engine version is used.
        ...
        # @return [Object] Specifies the number of IOPS for the instance.  Used only when command=create or command=modify. Must be an integer greater than 1000.
        attribute :iops

        # @return [Object] Comma separated list of one or more security groups.  Used only when command=create or command=modify.
        attribute :security_groups

        # @return [String] Comma separated list of one or more vpc security group ids. Also requires `subnet` to be specified. Used only when command=create or command=modify.
        attribute :vpc_security_groups
        validates :vpc_security_groups, type: String

        # @return [Array<String>] Port number that the DB instance uses for connections. Used only when command=create or command=replicate.,Prior to 2.0 it always defaults to null and the API would use 3306, it had to be set to other DB default values when not using MySql. Starting at 2.0 it auotmaticaly defaults to what is expected for each c(db_engine).
        ...
        # @return [Object] The name of the option group to use.  If not specified then the default option group is used. Used only when command=create.
        attribute :option_group

        # @return [Object] Maintenance window in format of ddd:hh24:mi-ddd:hh24:mi.  (Example: Mon:22:00-Mon:23:15) If not specified then a random maintenance window is assigned. Used only when command=create or command=modify.
        attribute :maint_window

        # @return [Object] Backup window in format of hh24:mi-hh24:mi.  If not specified then a random backup window is assigned. Used only when command=create or command=modify.
        attribute :backup_window

        # @return [Object] Number of days backups are retained.  Set to 0 to disable backups.  Default is 1 day.  Valid range: 0-35. Used only when command=create or command=modify.
        attribute :backup_retention

        # @return [String] availability zone in which to launch the instance. Used only when command=create, command=replicate or command=restore.
        attribute :zone
        validates :zone, type: String

        # @return [String] VPC subnet group.  If specified then a VPC instance is created. Used only when command=create.
        attribute :subnet
        validates :subnet, type: String

        # @return [String] Name of snapshot to take. When command=delete, if no snapshot name is provided then no snapshot is taken. If used with command=delete with no instance_name, the snapshot is deleted. Used with command=facts, command=delete or command=snapshot.
        attribute :snapshot
        ...
        # @return [String] Name to rename an instance to. Used only when command=modify.
        attribute :new_instance_name
        validates :new_instance_name, type: String

        # @return [Object] Associate the DB instance with a specified character set. Used with command=create.
        attribute :character_set_name

        # @return [TrueClass] explicitly set whether the resource should be publicly accessible or not. Used with command=create, command=replicate. Requires boto >= 2.26.0
        attribute :publicly_accessible
        validates :publicly_accessible, type: TrueClass

        # @return [Hash] tags dict to apply to a resource. Used with command=create, command=replicate, command=restore. Requires boto >= 2.26.0
        attribute :tags
