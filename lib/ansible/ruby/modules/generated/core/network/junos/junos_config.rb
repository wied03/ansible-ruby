  ...
  module Ruby
    module Modules
      class Junos_config < Base
        # @return [Array, nil] The path to the config source.  The source can be either a file with config or a template that will be merged during runtime.  By default the task will search for the source file in role or playbook root folder in templates directory.
        attribute :lines
        ...
        # @return [TrueClass, nil] The C(zeroize) argument is used to completely ssantaize the remote device configuration back to initial defaults.  This argument will effectively remove all current configuration statements on the remote device
        attribute :zeroize
