  ...
  module Ruby
    module Modules
      class Eos_template < Base
        ...
        # @return [Boolean, nil] By default when the M(eos_template) connects to the remote device to retrieve the configuration it will issue the `show running-config` command.  If this option is set to True then the issued command will be `show running-config all`
        ...
        # @return [Boolean, nil] This argument will cause the provided configuration to be replaced on the destination node.   The use of the replace argument will always cause the task to set changed to true and will implies I(force) is true.  This argument is only valid with I(transport) is eapi.
        attribute :replace
