  ...
  module Ruby
    module Modules
      class Nxos_switchport < Base
        ...
        # @return [String, nil] if mode=access, used as the access vlan id
        attribute :access_vlan
        validates :access_vlan, type: String

        # @return [String, nil] if mode=trunk, used as the trunk native vlan id
        attribute :native_vlan
        validates :native_vlan, type: String

        # @return [String, nil] if mode=trunk, used as the vlan range to ADD or REMOVE from the trunk
