  ...
  module Ruby
    module Modules
      ...
      class Nxos_ntp_auth < Base
        # @return [Integer] Authentication key identifier (numeric).
        attribute :key_id
        validates :key_id, presence: true, type: Integer

        # @return [String] MD5 String.
        attribute :md5string
        validates :md5string, presence: true, type: String
        ...
        # @return [Boolean, nil] Whether the given key is required to be supplied by a time source for the device to synchronize to the time source.
        attribute :trusted_key
        validates :trusted_key, inclusion: {:in=>[true, false], :message=>"%{value} needs to be true, false"}, allow_nil: true
