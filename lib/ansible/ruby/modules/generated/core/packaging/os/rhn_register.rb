  ...
  module Ruby
    module Modules
      class Rhn_register < Base
        ...
        # @return [String, nil] supply a custom ssl CA certificate file for use with registration
        attribute :sslcacert
        validates :sslcacert, type: String

        # @return [String, nil] supply an organizational id for use with registration
        attribute :systemorgid
        validates :systemorgid, type: String

        # @return [Array, nil] Optionally specify a list of comma-separated channels to subscribe to upon successful registration.
        attribute :channels
