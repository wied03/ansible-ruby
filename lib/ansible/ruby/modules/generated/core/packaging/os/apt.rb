...
module Ansible
  module Ruby
    module Modules
      class Apt < Base
        # @return [String] A package name, like C(foo), or package specifier with version, like C(foo=1.0). Name wildcards (fnmatch) like C(apt*) and version wildcards like C(foo=1.0*) are also supported.  Note that the apt-get commandline supports implicit regex matches here but we do not because it can let typos through easier (If you typo C(foo) as C(fo) apt-get would install packages that have "fo" in their name with a warning and a prompt for the user.  Since we don't have warnings and prompts before installing we disallow this.  Use an explicit fnmatch pattern if you want wildcarding)
        attribute :name
        ...
        # @return [String] If C(update_cache) is specified and the last run is less or equal than I(cache_valid_time) seconds ago, the C(update_cache) gets skipped.
        ...
        # @return [String] Corresponds to the C(-t) option for I(apt) and sets pin priorities
        ...
        # @return [Array<String>] Add dpkg options to apt command. Defaults to '-o "Dpkg::Options::=--force-confdef" -o "Dpkg::Options::=--force-confold"',Options should be supplied as comma separated list
        attribute :dpkg_options, flat_array: true
        validates :dpkg_options, type: TypeGeneric.new(String)

        # @return [String] Path to a .deb package on the remote machine.,If :// in the path, ansible will attempt to download deb before installing. (Version added 2.1)
        attribute :deb
        ...
        # @return [Object] Only install/upgrade a package if it is already installed.
