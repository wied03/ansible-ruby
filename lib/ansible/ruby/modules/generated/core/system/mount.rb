...
module Ansible
  module Ruby
    module Modules
      ...
      class Mount < Base
        # @return [String] Path to the mount point (e.g. C(/mnt/files))
        attribute :name
        validates :name, presence: true, type: String

        # @return [String, nil] Device to be mounted on I(name). Required when I(state) set to C(present) or C(mounted).
        attribute :src
