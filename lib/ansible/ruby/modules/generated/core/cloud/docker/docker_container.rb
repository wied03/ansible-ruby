  ...
  module Ruby
    module Modules
      class Docker_container < Base
        ...
        # @return [TrueClass, nil] Enable detached mode to leave the container running in background. If disabled, the task will reflect the status of the container run (failed if the command failed).
        attribute :detach
        validates :detach, type: TrueClass

        # @return [Array, nil] List of host device bindings to add to the container. Each binding is a mapping expressed in the format: <path_on_host>:<path_in_container>:<cgroup_permissions>
        attribute :devices
        ...
        # @return [Array, nil] List of additional container ports to expose for port mappings or links. If the port is already exposed using EXPOSE in a Dockerfile, it does not need to be xposed again.
        attribute :exposed_ports
        validates :exposed_ports, type: Array

        # @return [Object, nil] Use the kill command when stopping a running container.
        ...
        # @return [Object, nil] Keep stdin open after a container is launched, even if not attached.
        ...
        # @return [TrueClass, nil] Retain volumes associated with a removed container.
        ...
        # @return [Array, nil] List of name aliases for linked containers in the format C(container_name:alias)
        attribute :links
        ...
        # @return [Object, nil] If true, always pull the latest version of an image. Otherwise, will only pull an image when missing.
        attribute :pull

        # @return [TrueClass, nil] Remove the container from ALL networks not included in C(networks) parameter.,Any default networks such as I(bridge), if not found in C(networks), will be removed as well.
        attribute :purge_networks
        validates :purge_networks, type: TrueClass

        # @return [Object, nil] Mount the container's root file system as read-only.
        attribute :read_only

        # @return [TrueClass, nil] Use with present and started states to force the re-creation of an existing container.
        attribute :recreate
        validates :recreate, type: TrueClass

        # @return [TrueClass, nil] Use with started state to force a matching container to be stopped and restarted.
        attribute :restart
        ...
        # @return [Object, nil] If true, skip image verification.
        attribute :trust_image_content

        # @return [Object, nil] Allocate a psuedo-TTY.
        attribute :tty
        ...
        # @return [Array, nil] List of volumes to mount within the container.,Use docker CLI-style syntax: C(/host:/container[:mode]),You can specify a read mode for the mount with either C(ro) or C(rw).,SELinux hosts can additionally use C(z) or C(Z) to use a shared or private label for the volume.
        attribute :volumes
        ...
        # @return [Array, nil] List of container names or Ids to get volumes from.
