  ...
  module Ruby
    module Modules
      ...
      class Docker < Base
        ...
        # @return [Boolean, nil] Publish all exposed ports to the host interfaces.
        ...
        # @return [Boolean, nil] Enable detached mode to leave the container running in background. If disabled, fail unless the process exits cleanly.
        attribute :detach
        ...
        # @return [Boolean, nil] Whether the container should run in privileged mode or not.
        ...
        # @return [Boolean, nil] Keep stdin open after a container is launched.
        attribute :stdin_open
        validates :stdin_open, inclusion: {:in=>[true, false], :message=>"%{value} needs to be true, false"}, allow_nil: true

        # @return [Boolean, nil] Allocate a pseudo-tty within the container.
        attribute :tty
        validates :tty, inclusion: {:in=>[true, false], :message=>"%{value} needs to be true, false"}, allow_nil: true

        # @return [Boolean, nil] Network mode for the launched container: bridge, none, container:<name|id>,or host. Requires docker >= 0.11.
        attribute :net
        validates :net, inclusion: {:in=>[true, false], :message=>"%{value} needs to be true, false"}, allow_nil: true

        # @return [Object, nil] Set the PID namespace mode for the container (currently only supports 'host'). Requires docker-py >= 1.0.0 and docker >= 1.5.0
        attribute :pid
        ...
        # @return [Boolean, nil] Use insecure private registry by HTTP instead of HTTPS. Needed for docker-py >= 0.5.0.
        attribute :insecure_registry
        validates :insecure_registry, inclusion: {:in=>[true, false], :message=>"%{value} needs to be true, false"}, allow_nil: true

        # @return [Object, nil] CPUs in which to allow execution. Requires docker-py >= 0.6.0.
        attribute :cpu_set

        # @return [Boolean, nil] Add capabilities for the container. Requires docker-py >= 0.5.0.
        attribute :cap_add
        validates :cap_add, inclusion: {:in=>[true, false], :message=>"%{value} needs to be true, false"}, allow_nil: true

        # @return [Boolean, nil] Drop capabilities for the container. Requires docker-py >= 0.5.0.
        attribute :cap_drop
        validates :cap_drop, inclusion: {:in=>[true, false], :message=>"%{value} needs to be true, false"}, allow_nil: true
