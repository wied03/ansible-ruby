  ...
  module Ruby
    module Modules
      class Azure_rm_virtualmachine < Base
        ...
        # @return [TrueClass, nil] Use with state 'present' to start the machine. Set to false to have the machine be 'stopped'.
        attribute :started
        validates :started, type: TrueClass

        # @return [TrueClass, nil] Toggle that controls if the machine is allocated/deallocated, only useful with state='present'.
        attribute :allocated
        validates :allocated, type: TrueClass

        # @return [TrueClass, nil] Use with state 'present' to restart a running VM.
        attribute :restarted
        ...
        # @return [TrueClass, nil] When the os_type is Linux, setting ssh_password_enabled to false will disable SSH password authentication and require use of SSH keys.
        ...
        # @return [Array, nil] When removing a VM using state 'absent', also remove associated resources,It can be 'all' or a list with any of the following: ['network_interfaces', 'virtual_storage', 'public_ips'],Any other input will be ignored
