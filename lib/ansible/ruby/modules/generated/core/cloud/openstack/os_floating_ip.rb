  ...
  module Ruby
    module Modules
      class Os_floating_ip < Base
        ...
        # @return [TrueClass, nil] When I(state) is present, and I(floating_ip_address) is not present, this parameter can be used to specify whether we should try to reuse a floating IP address already allocated to the project.
        attribute :reuse
        ...
        # @return [TrueClass, nil] When attaching a floating IP address, specify whether we should wait for it to appear as attached.
        attribute :wait
        ...
        # @return [Object, nil] When I(state) is absent, indicates whether or not to delete the floating IP completely, or only detach it from the server. Default is to detach only.
        attribute :purge
      end
    end
  end
