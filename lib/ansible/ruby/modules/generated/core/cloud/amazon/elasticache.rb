  ...
  module Ruby
    module Modules
      class Elasticache < Base
        ...
        # @return [String, nil] The port number on which each of the cache nodes will accept connections
        attribute :cache_port
        validates :cache_port, type: String

        # @return [String, nil] The name of the cache parameter group to associate with this cache cluster. If this argument is omitted, the default cache parameter group for the specified engine will be used.
        attribute :cache_parameter_group
        validates :cache_parameter_group, type: String

        # @return [String, nil] The subnet group name to associate with. Only use if inside a vpc. Required if inside a vpc
        attribute :cache_subnet_group
        validates :cache_subnet_group, type: String

        # @return [String, nil] A list of vpc security group names to associate with this cache cluster. Only use if inside a vpc
        attribute :security_group_ids
        validates :security_group_ids, type: String

        # @return [String, nil] A list of cache security group names to associate with this cache cluster. Must be an empty list if inside a vpc
