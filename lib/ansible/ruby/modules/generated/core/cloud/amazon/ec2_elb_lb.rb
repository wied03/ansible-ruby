  ...
  module Ruby
    module Modules
      class Ec2_elb_lb < Base
        ...
        # @return [Array, nil] List of ports/protocols for this ELB to listen on (see example)
        attribute :listeners
        validates :listeners, type: Array

        # @return [TrueClass, nil] Purge existing listeners on ELB that are not found in listeners
        attribute :purge_listeners
        validates :purge_listeners, type: TrueClass

        # @return [Array, nil] List of instance ids to attach to this ELB
        attribute :instance_ids
        validates :instance_ids, type: Array

        # @return [TrueClass, nil] Purge existing instance ids on ELB that are not found in instance_ids
        attribute :purge_instance_ids
        validates :purge_instance_ids, type: TrueClass

        # @return [Array, nil] List of availability zones to enable on this ELB
        attribute :zones
        validates :zones, type: Array

        # @return [TrueClass, nil] Purge existing availability zones on ELB that are not found in zones
        attribute :purge_zones
        validates :purge_zones, type: TrueClass

        # @return [String, nil] A list of security groups to apply to the elb
        attribute :security_group_ids
        validates :security_group_ids, type: String

        # @return [String, nil] A list of security group names to apply to the elb
        attribute :security_group_names
        validates :security_group_names, type: String

        # @return [String, nil] An associative array of health check configuration settings (see example)
        attribute :health_check
        validates :health_check, type: String

        # @return [String, nil] An associative array of access logs configuration settings (see example)
        attribute :access_logs
        validates :access_logs, type: String

        # @return [String, nil] A list of VPC subnets to use when creating ELB. Zones should be empty if using this.
        attribute :subnets
        validates :subnets, type: String

        # @return [TrueClass, nil] Purge existing subnet on ELB that are not found in subnets
