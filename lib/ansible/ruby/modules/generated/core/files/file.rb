  ...
  module Ruby
    module Modules
      class File < Base
        ...
        # @return [String] recursively set the specified file attributes (applies only to state=directory)
        attribute :recurse
        validates :recurse, inclusion: {:in=>[:yes, :no], :message=>"%{value} needs to be :yes, :no"}, allow_nil: true

        # @return [String] force the creation of the symlinks in two cases: the source file does not exist (but will appear later); the destination exists and is a file (so, we need to unlink the "path" file and create symlink to the "src" file in place of it).
        attribute :force
