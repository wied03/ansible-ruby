  ...
  module Ruby
    module Modules
      ...
      class Bigip_profile_http < Base
        ...
        validates :proxy_type, inclusion: {:in=>[:reverse, :transparent, :explicit], :message=>"%{value} needs to be :reverse, :transparent, :explicit"}, allow_nil: true
        ...
        # @return [Boolean, nil] When specified system inserts an X-Forwarded-For header in an HTTP request with the client IP address, to use with connection pooling.,When creating a new profile, if this parameter is not specified, the default is provided by the parent profile.
        attribute :insert_xforwarded_for
        validates :insert_xforwarded_for, inclusion: {:in=>[true, false], :message=>"%{value} needs to be true, false"}, allow_nil: true
        ...
        validates :redirect_rewrite, inclusion: {:in=>[:none, :all, :matching, :nodes], :message=>"%{value} needs to be :none, :all, :matching, :nodes"}, allow_nil: true

        # @return [Object, nil] Cookie names for the system to encrypt.,To remove the entry completely a value of C(none) or C('') should be set.,When creating a new profile, if this parameter is not specified, the default is provided by the parent profile.
