  ...
  module Ruby
    module Modules
      ...
      class Ce_vxlan_global < Base
        ...
        validates :tunnel_mode_vxlan, inclusion: {:in=>[:enable, :disable], :message=>"%{value} needs to be :enable, :disable"}, allow_nil: true
        ...
        validates :nvo3_prevent_loops, inclusion: {:in=>[:enable, :disable], :message=>"%{value} needs to be :enable, :disable"}, allow_nil: true

        # @return [:enable, :disable, nil] Enabling or disabling the VXLAN ACL extension function.
        attribute :nvo3_acl_extend
        validates :nvo3_acl_extend, inclusion: {:in=>[:enable, :disable], :message=>"%{value} needs to be :enable, :disable"}, allow_nil: true

        # @return [:l2, :l3, nil] Configuring the Layer 3 VXLAN Gateway to Work in Non-loopback Mode.
        attribute :nvo3_gw_enhanced
        validates :nvo3_gw_enhanced, inclusion: {:in=>[:l2, :l3], :message=>"%{value} needs to be :l2, :l3"}, allow_nil: true

        # @return [:enable, :disable, nil] Enabling or disabling the VXLAN service extension function.
        attribute :nvo3_service_extend
        validates :nvo3_service_extend, inclusion: {:in=>[:enable, :disable], :message=>"%{value} needs to be :enable, :disable"}, allow_nil: true

        # @return [:enable, :disable, nil] Eth-Trunk from load balancing VXLAN packets in optimized mode.
        attribute :nvo3_eth_trunk_hash
        validates :nvo3_eth_trunk_hash, inclusion: {:in=>[:enable, :disable], :message=>"%{value} needs to be :enable, :disable"}, allow_nil: true

        # @return [:enable, :disable, nil] Load balancing of VXLAN packets through ECMP in optimized mode.
        attribute :nvo3_ecmp_hash
        validates :nvo3_ecmp_hash, inclusion: {:in=>[:enable, :disable], :message=>"%{value} needs to be :enable, :disable"}, allow_nil: true
