  ...
  module Ruby
    module Modules
      ...
      class Ce_info_center_global < Base
        ...
        validates :info_center_enable, inclusion: {:in=>[:true, :false], :message=>"%{value} needs to be :true, :false"}, allow_nil: true
        ...
        validates :suppress_enable, inclusion: {:in=>[:false, :true], :message=>"%{value} needs to be :false, :true"}, allow_nil: true
        ...
        validates :logfile_max_size, inclusion: {:in=>[4, 8, 16, 32], :message=>"%{value} needs to be 4, 8, 16, 32"}, allow_nil: true
        ...
        validates :channel_out_direct, inclusion: {:in=>[:console, :monitor, :trapbuffer, :logbuffer, :snmp, :logfile], :message=>"%{value} needs to be :console, :monitor, :trapbuffer, :logbuffer, :snmp, :logfile"}, allow_nil: true
        ...
        attribute :ip_type
        validates :ip_type, inclusion: {:in=>[:ipv4, :ipv6], :message=>"%{value} needs to be :ipv4, :ipv6"}, allow_nil: true
        ...
        attribute :level
        validates :level, inclusion: {:in=>[:emergencies, :alert, :critical, :error, :warning, :notification, :informational, :debugging], :message=>"%{value} needs to be :emergencies, :alert, :critical, :error, :warning, :notification, :informational, :debugging"}, allow_nil: true
        ...
        attribute :facility
        validates :facility, inclusion: {:in=>[:local0, :local1, :local2, :local3, :local4, :local5, :local6, :local7], :message=>"%{value} needs to be :local0, :local1, :local2, :local3, :local4, :local5, :local6, :local7"}, allow_nil: true
        ...
        attribute :timestamp
        validates :timestamp, inclusion: {:in=>[:UTC, :localtime], :message=>"%{value} needs to be :UTC, :localtime"}, allow_nil: true

        # @return [:tcp, :udp, nil] Transport mode. The value is of the enumerated type and case-sensitive.
        attribute :transport_mode
        validates :transport_mode, inclusion: {:in=>[:tcp, :udp], :message=>"%{value} needs to be :tcp, :udp"}, allow_nil: true
