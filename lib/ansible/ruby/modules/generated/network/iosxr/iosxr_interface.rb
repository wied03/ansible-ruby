  ...
  module Ruby
    module Modules
      ...
      class Iosxr_interface < Base
        ...
        attribute :active
        validates :active, inclusion: {:in=>[:active, :preconfigure], :message=>"%{value} needs to be :active, :preconfigure"}, allow_nil: true

        # @return [10, 100, 1000, nil] Configure the speed for an interface. Default is auto-negotiation when not configured.
        attribute :speed
        validates :speed, inclusion: {:in=>[10, 100, 1000], :message=>"%{value} needs to be 10, 100, 1000"}, allow_nil: true
        ...
        attribute :duplex
        validates :duplex, inclusion: {:in=>[:full, :half], :message=>"%{value} needs to be :full, :half"}, allow_nil: true
