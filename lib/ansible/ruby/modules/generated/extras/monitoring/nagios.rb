  ...
  module Ruby
    module Modules
      class Nagios < Base
        ...
        # @return [String] Host to operate on in Nagios.
        attribute :host
        validates :host, type: String

        # @return [String] Path to the nagios I(command file) (FIFO pipe). Only required if auto-detection fails.
        attribute :cmdfile
        validates :cmdfile, type: String

        # @return [String] Author to leave downtime comments as. Only usable with the C(downtime) action.
        attribute :author
        validates :author, type: String

        # @return [String] Comment for C(downtime) action.
        attribute :comment
        validates :comment, type: String

        # @return [Fixnum] Minutes to schedule downtime for.,Only usable with the C(downtime) action.
        attribute :minutes
        ...
        # @return [String] The Servicegroup we want to set downtimes/alerts for. B(Required) option when using the C(servicegroup_service_downtime) amd C(servicegroup_host_downtime).
