  ...
  module Ruby
    module Modules
      class Zabbix_maintenance < Base
        ...
        # @return [Array<String>, nil] Hosts to manage maintenance window for. Separate multiple hosts with commas. C(host_name) is an alias for C(host_names). B(Required) option when C(state) is I(present) and no C(host_groups) specified.
        attribute :host_names, flat_array: true
        validates :host_names, type: TypeGeneric.new(String)

        # @return [Array<String>, nil] Host groups to manage maintenance window for. Separate multiple groups with commas. C(host_group) is an alias for C(host_groups). B(Required) option when C(state) is I(present) and no C(host_names) specified.
