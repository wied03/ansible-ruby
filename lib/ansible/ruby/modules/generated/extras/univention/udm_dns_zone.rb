  ...
  module Ruby
    module Modules
      ...
      class Udm_dns_zone < Base
        ...
        # @return [String, nil] List of appropriate name servers. Required if C(state=present).
        attribute :nameserver
        validates :nameserver, type: String

        # @return [String, nil] List of interface IP addresses, on which the server should response this zone. Required if C(state=present).
