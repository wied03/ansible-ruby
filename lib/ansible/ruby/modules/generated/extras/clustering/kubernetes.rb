  ...
  module Ruby
    module Modules
      class Kubernetes < Base
        ...
        # @return [String] Specify full path to a Kubernets YAML file to send to API I(endpoint). This option is mutually exclusive with C('inline_data').
        ...
        # @return [String] The HTTP Basic Auth password for the API I(endpoint). This should be set unless using the C('insecure') option.
        attribute :url_password
        validates :url_password, type: String

        # @return [String] The HTTP Basic Auth username for the API I(endpoint). This should be set unless using the C('insecure') option.
        attribute :url_username
        validates :url_username, type: String

        # @return [TrueClass] Reverts the connection to using HTTP instead of HTTPS. This option should only be used when execuing the M('kubernetes') module local to the Kubernetes cluster using the insecure local port (locahost:8080 by default).
        attribute :insecure
        validates :insecure, type: TrueClass

        # @return [Object] Enable/disable certificate validation. Note that this is set to C(false) until Ansible can support IP address based certificate hostname matching (exists in >= python3.5.0).
