  ...
  module Ruby
    module Modules
      class Iptables < Base
        ...
        # @return [String, nil] Destination port or port range specification. This can either be a service name or a port number. An inclusive range can also be specified, using the format first:last. If the first port is omitted, '0' is assumed; if the last is omitted, '65535' is assumed. If the first port is greater than the second one they will be swapped.
        attribute :destination_port
        validates :destination_port, type: String

        # @return [String, nil] This specifies a destination port or range of ports to use: without this, the destination port is never altered. This is only valid if the rule also specifies one of the following protocols: tcp, udp, dccp or sctp.
        attribute :to_ports
        ...
        # @return [Array, nil] ctstate is a list of the connection states to match in the conntrack module. Possible states are: 'INVALID', 'NEW', 'ESTABLISHED', 'RELATED', 'UNTRACKED', 'SNAT', 'DNAT'
        attribute :ctstate
        ...
        # @return [Object, nil] Specifies the UID or username to use in match by owner rule.
        attribute :uid_owner
