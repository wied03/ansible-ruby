  ...
  module Ruby
    module Modules
      class Selinux_permissive < Base
        ...
        # @return [String] indicate if the domain should or should not be set as permissive
        attribute :permissive
        validates :permissive, presence: true, inclusion: {:in=>[:True, :False], :message=>"%{value} needs to be :True, :False"}

        # @return [Object] automatically reload the policy after a change,default is set to 'false' as that's what most people would want after changing one domain,Note that this doesn't work on older version of the library (example EL 6), the module will silently ignore it in this case
        attribute :no_reload
