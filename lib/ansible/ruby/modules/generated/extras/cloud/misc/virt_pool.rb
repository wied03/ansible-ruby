  ...
  module Ruby
    module Modules
      class Virt_pool < Base
        ...
        # @return [String] specify which state you want a storage pool to be in. If 'active', pool will be started. If 'present', ensure that pool is present but do not change its state; if it's missing, you need to specify xml argument. If 'inactive', pool will be stopped. If 'undefined' or 'absent', pool will be removed from I(libvirt) configuration. If 'deleted', pool contents will be deleted and then pool undefined.
        attribute :state
        ...
        # @return [String] Specify if a given storage pool should be started automatically on system boot.
        attribute :autostart
        ...
        # @return [Object] Pass additional parameters to 'build' or 'delete' commands.
        attribute :mode
