...
module Ansible
  module Ruby
    module Modules
      class Ec2_eni < Base
        # @return [Hash, nil] The ID of the ENI
        attribute :eni_id
        ...
        # @return [TrueClass, nil] Force detachment of the interface. This applies either when explicitly detaching the interface by setting instance_id to None or when deleting an interface with state=absent.
        attribute :force_detach
        validates :force_detach, type: TrueClass

        # @return [TrueClass, nil] Delete the interface when the instance it is attached to is terminated. You can only specify this flag when the interface is being modified, not on creation.
