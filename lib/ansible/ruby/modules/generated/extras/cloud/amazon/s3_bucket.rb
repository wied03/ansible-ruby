  ...
  module Ruby
    module Modules
      class S3_bucket < Base
        # @return [TrueClass] When trying to delete a bucket, delete all keys in the bucket first (an s3 bucket must be empty for a successful deletion)
        attribute :force
        ...
        # @return [TrueClass] With Requester Pays buckets, the requester instead of the bucket owner pays the cost of the request and the data download from the bucket.
        attribute :requester_pays
        validates :requester_pays, inclusion: {:in=>[:yes, :no], :message=>"%{value} needs to be :yes, :no"}, allow_nil: true

        # @return [String] Create or remove the s3 bucket
        attribute :state
        ...
        # @return [TrueClass] Whether versioning is enabled or disabled (note that once versioning is enabled, it can only be suspended)
