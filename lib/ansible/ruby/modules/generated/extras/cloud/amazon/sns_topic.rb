  ...
  module Ruby
    module Modules
      class Sns_topic < Base
        ...
        # @return [String, nil] Policy to apply to the SNS topic
        attribute :policy
        validates :policy, type: String

        # @return [String, nil] Delivery policy to apply to the SNS topic
        attribute :delivery_policy
        validates :delivery_policy, type: String

        # @return [Array, nil] List of subscriptions to apply to the topic. Note that AWS requires subscriptions to be confirmed, so you will need to confirm any new subscriptions.
        attribute :subscriptions
        validates :subscriptions, type: Array

        # @return [TrueClass, nil] Whether to purge any subscriptions not listed here. NOTE: AWS does not allow you to purge any PendingConfirmation subscriptions, so if any exist and would be purged, they are silently skipped. This means that somebody could come back later and confirm the subscription. Sorry. Blame Amazon.
