  ...
  module Ruby
    module Modules
      class Urpmi < Base
        ...
        # @return [String] update the package database first C(urpmi.update -a).
        attribute :update_cache
        validates :update_cache, inclusion: {:in=>[:yes, :no], :message=>"%{value} needs to be :yes, :no"}, allow_nil: true

        # @return [TrueClass] Corresponds to the C(--no-recommends) option for I(urpmi).
        attribute :no-recommends
        validates :no-recommends, inclusion: {:in=>[:yes, :no], :message=>"%{value} needs to be :yes, :no"}, allow_nil: true

        # @return [TrueClass] Assume "yes" is the answer to any question urpmi has to ask. Corresponds to the C(--force) option for I(urpmi).
        attribute :force
