  ...
  module Ruby
    module Modules
      class Bigip_pool_member < Base
        ...
        # @return [Object] Set new session availability status for pool member
        attribute :session_state
        validates :session_state, inclusion: {:in=>[:enabled, :disabled], :message=>"%{value} needs to be :enabled, :disabled"}, allow_nil: true

        # @return [Object] Set monitor availability status for pool member
        ...
        # @return [String] When state is absent and the pool member is no longer referenced in other pools, the default behavior removes the unused node object. Setting this to 'yes' disables this behavior.
