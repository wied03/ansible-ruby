  ...
  module Ruby
    module Modules
      class Nmcli < Base
        ...
        # @return [String, nil] master <master (ifname, or connection UUID or conn_name) of bridge, team, bond master connection profile.
        attribute :master
        validates :master, type: String

        # @return [String, nil] The IPv4 address to this interface using this format ie: "192.168.1.24/24"
        attribute :ip4
        ...
        # @return [String, nil] A list of upto 3 dns servers, ipv4 format e.g. To add two IPv4 DNS server addresses: ["8.8.8.8 8.8.4.4"]
        attribute :dns4
        validates :dns4, type: String

        # @return [String, nil] The IPv6 address to this interface using this format ie: "abbe::cafe"
        attribute :ip6
        validates :ip6, type: String

        # @return [String, nil] The IPv6 gateway for this interface using this format ie: "2001:db8::1"
        attribute :gw6
        ...
        # @return [String, nil] This is only used with bond and is the primary interface name (for "active-backup" mode), this is the usually the 'ifname'
        attribute :primary
        ...
        # @return [String, nil] This is only used with bond - downdelay
        attribute :downdelay
        validates :downdelay, type: String

        # @return [String, nil] This is only used with bond - updelay
        attribute :updelay
        validates :updelay, type: String

        # @return [String, nil] This is only used with bond - ARP interval
        attribute :arp_interval
        validates :arp_interval, type: String

        # @return [String, nil] This is only used with bond - ARP IP target
        attribute :arp_ip_target
        validates :arp_ip_target, type: String

        # @return [String, nil] This is only used with bridge and controls whether Spanning Tree Protocol (STP) is enabled for this bridge
        attribute :stp
        ...
        # @return [String, nil] This is only used with bridge - MAC address of the bridge (note: this requires a recent kernel feature, originally introduced in 3.15 upstream kernel)
        attribute :mac
        ...
        # @return [TrueClass, nil] This is only used with 'bridge-slave' - 'hairpin mode' for the slave, which allows frames to be sent back out through the slave the frame was received on.
        attribute :hairpin
        validates :hairpin, type: TrueClass

        # @return [String, nil] This is only used with VLAN - VLAN ID in range <0-4095>
        attribute :vlanid
        validates :vlanid, type: String

        # @return [String, nil] This is only used with VLAN - parent device this VLAN is on, can use ifname
        attribute :vlandev
        validates :vlandev, type: String

        # @return [String, nil] This is only used with VLAN - flags
        attribute :flags
        validates :flags, type: String

        # @return [String, nil] This is only used with VLAN - VLAN ingress priority mapping
        attribute :ingress
        validates :ingress, type: String

        # @return [String, nil] This is only used with VLAN - VLAN egress priority mapping
        attribute :egress
