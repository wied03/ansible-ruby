  ...
  module Ruby
    module Modules
      ...
      class Win_certificate_store < Base
        ...
        attribute :state
        validates :state, expression_inclusion: {:in=>[:absent, :exported, :present], :message=>"%{value} needs to be :absent, :exported, :present"}, allow_nil: true
        ...
        validates :store_name, expression_inclusion: {:in=>[:AddressBook, :AuthRoot, :CertificateAuthority, :Disallowed, :My, :Root, :TrustedPeople, :TrustedPublisher], :message=>"%{value} needs to be :AddressBook, :AuthRoot, :CertificateAuthority, :Disallowed, :My, :Root, :TrustedPeople, :TrustedPublisher"}, allow_nil: true
        ...
        validates :store_location, expression_inclusion: {:in=>[:CurrentUser, :LocalMachine], :message=>"%{value} needs to be :CurrentUser, :LocalMachine"}, allow_nil: true
        ...
        validates :key_exportable, expression_inclusion: {:in=>[:yes, :no], :message=>"%{value} needs to be :yes, :no"}, allow_nil: true
        ...
        validates :key_storage, expression_inclusion: {:in=>[:default, :machine, :user], :message=>"%{value} needs to be :default, :machine, :user"}, allow_nil: true
        ...
        attribute :file_type
        validates :file_type, expression_inclusion: {:in=>[:der, :pem, :pkcs12], :message=>"%{value} needs to be :der, :pem, :pkcs12"}, allow_nil: true
      end
    end
  end
end
