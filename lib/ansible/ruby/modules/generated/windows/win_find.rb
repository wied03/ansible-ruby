  ...
  module Ruby
    module Modules
      ...
      class Win_find < Base
        ...
        # @return [:atime, :mtime, :ctime, nil] Choose the file property against which we compare C(age). The default attribute we compare with is the last modification time.
        attribute :age_stamp
        ...
        # @return [:file, :directory, nil] Type of file to search for
        attribute :file_type
        validates :file_type, inclusion: {:in=>[:file, :directory], :message=>"%{value} needs to be :file, :directory"}, allow_nil: true

        # @return [Boolean, nil] Set this to true to follow symlinks in the path. This needs to be used in conjunction with C(recurse).
        attribute :follow
        validates :follow, inclusion: {:in=>[true, false], :message=>"%{value} needs to be true, false"}, allow_nil: true

        # @return [Boolean, nil] Whether to return a checksum of the file in the return info (default sha1), use C(checksum_algorithm) to change from the default.
        attribute :get_checksum
        validates :get_checksum, inclusion: {:in=>[true, false], :message=>"%{value} needs to be true, false"}, allow_nil: true

        # @return [Boolean, nil] Set this to include hidden files or folders
        attribute :hidden
        ...
        # @return [Boolean, nil] Will recursively descend into the directory looking for files or folders
        attribute :recurse
        validates :recurse, inclusion: {:in=>[true, false], :message=>"%{value} needs to be true, false"}, allow_nil: true

        # @return [Boolean, nil] Select files or folders whose size is equal to or greater than the specified size. Use a negative value to find files equal to or less than the specified size. You can specify the size with a suffix of the byte type i.e. kilo = k, mega = m... Size is not evaluated for symbolic links.
        attribute :size
        validates :size, inclusion: {:in=>[true, false], :message=>"%{value} needs to be true, false"}, allow_nil: true

        # @return [Boolean, nil] Will set patterns to run as a regex check if true
        attribute :use_regex
