  ...
  module Ruby
    module Modules
      class Cloudformation < Base
        ...
        validates :template_parameters, type: Hash
        ...
        # @return [String] The local path of the cloudformation template. This parameter is mutually exclusive with 'template_url'. Either one of them is required if "state" parameter is "present" Must give full path to the file, relative to the working directory. If using roles this may look like "roles/cloudformation/files/cloudformation-example.json"
        attribute :template
        validates :template, type: String

        # @return [Object] The Simple Notification Service (SNS) topic ARNs to publish stack related events.
        attribute :notification_arns

        # @return [Object] the path of the cloudformation stack policy
        attribute :stack_policy

        # @return [Hash] Dictionary of tags to associate with stack and it's resources during stack creation. Cannot be updated later. Requires at least Boto version 2.6.0.
        attribute :tags
        ...
        # @return [String] Location of file containing the template body. The URL must point to a template (max size 307,200 bytes) located in an S3 bucket in the same region as the stack. This parameter is mutually exclusive with 'template'. Either one of them is required if "state" parameter is "present"
