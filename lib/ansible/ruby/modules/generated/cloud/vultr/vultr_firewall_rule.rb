  ...
  module Ruby
    module Modules
      ...
      class Vultr_firewall_rule < Base
        ...
        validates :ip_version, inclusion: {:in=>[:v4, :v6], :message=>"%{value} needs to be :v4, :v6"}, allow_nil: true

        # @return [:icmp, :tcp, :udp, :gre, nil] Protocol of the firewall rule.
        attribute :protocol
        validates :protocol, inclusion: {:in=>[:icmp, :tcp, :udp, :gre], :message=>"%{value} needs to be :icmp, :tcp, :udp, :gre"}, allow_nil: true
