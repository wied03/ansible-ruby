...
module Ansible
  module Ruby
    module Modules
      ...
      class Ovirt_clusters < Base
        # @return [String] Name of the the cluster to manage.
        attribute :name
        ...
        validates :ballooning, inclusion: {:in=>[true, false], :message=>"%{value} needs to be true, false"}, allow_nil: true
        ...
        attribute :gluster
        validates :gluster, inclusion: {:in=>[true, false], :message=>"%{value} needs to be true, false"}, allow_nil: true
        ...
        validates :threads_as_cores, inclusion: {:in=>[true, false], :message=>"%{value} needs to be true, false"}, allow_nil: true
        ...
        # @return [Boolean, nil] If I(True) enables KSM C(ksm) for best berformance inside NUMA nodes.
        attribute :ksm_numa
        validates :ksm_numa, inclusion: {:in=>[true, false], :message=>"%{value} needs to be true, false"}, allow_nil: true

        # @return [Boolean, nil] If I(True) enable the oVirt to monitor cluster capacity for highly available virtual machines.
        attribute :ha_reservation
        validates :ha_reservation, inclusion: {:in=>[true, false], :message=>"%{value} needs to be true, false"}, allow_nil: true

        # @return [Boolean, nil] If (True) enable integration with an OpenAttestation server.
        attribute :trusted_service
        validates :trusted_service, inclusion: {:in=>[true, false], :message=>"%{value} needs to be true, false"}, allow_nil: true

        # @return [Boolean, nil] If I(True) enable an optional reason field when a virtual machine is shut down from the Manager, allowing the administrator to provide an explanation for the maintenance.
        attribute :vm_reason
        validates :vm_reason, inclusion: {:in=>[true, false], :message=>"%{value} needs to be true, false"}, allow_nil: true

        # @return [Boolean, nil] If I(True) enable an optional reason field when a host is placed into maintenance mode from the Manager, allowing the administrator to provide an explanation for the maintenance.
        attribute :host_reason
        validates :host_reason, inclusion: {:in=>[true, false], :message=>"%{value} needs to be true, false"}, allow_nil: true
        ...
        validates :memory_policy, inclusion: {:in=>[:disabled, :server, :desktop], :message=>"%{value} needs to be :disabled, :server, :desktop"}, allow_nil: true
        ...
        validates :resilience_policy, inclusion: {:in=>[:do_not_migrate, :migrate, :migrate_highly_available], :message=>"%{value} needs to be :do_not_migrate, :migrate, :migrate_highly_available"}, allow_nil: true
        ...
        validates :migration_bandwidth, inclusion: {:in=>[:auto, :hypervisor_default, :custom], :message=>"%{value} needs to be :auto, :hypervisor_default, :custom"}, allow_nil: true
        ...
        validates :migration_auto_converge, inclusion: {:in=>[:true, :false, :inherit], :message=>"%{value} needs to be :true, :false, :inherit"}, allow_nil: true
        ...
        validates :migration_compressed, inclusion: {:in=>[:true, :false, :inherit], :message=>"%{value} needs to be :true, :false, :inherit"}, allow_nil: true

        # @return [:legacy, :minimal_downtime, :suspend_workload, nil] A migration policy defines the conditions for live migrating virtual machines in the event of host failure.,Following policies are supported:,C(legacy) - Legacy behavior of 3.6 version.,C(minimal_downtime) - Virtual machines should not experience any significant downtime.,C(suspend_workload) - Virtual machines may experience a more significant downtime.
        attribute :migration_policy
        validates :migration_policy, inclusion: {:in=>[:legacy, :minimal_downtime, :suspend_workload], :message=>"%{value} needs to be :legacy, :minimal_downtime, :suspend_workload"}, allow_nil: true
        ...
        attribute :cpu_arch
        validates :cpu_arch, inclusion: {:in=>[:x86_64, :ppc64, :undefined], :message=>"%{value} needs to be :x86_64, :ppc64, :undefined"}, allow_nil: true

        # @return [String, nil] CPU codename. For example I(Intel SandyBridge Family).
        attribute :cpu_type
        validates :cpu_type, type: String

        # @return [:legacy, :ovs, nil] Type of switch to be used by all networks in given cluster. Either I(legacy) which is using linux brigde or I(ovs) using Open vSwitch.
        attribute :switch_type
        validates :switch_type, inclusion: {:in=>[:legacy, :ovs], :message=>"%{value} needs to be :legacy, :ovs"}, allow_nil: true
