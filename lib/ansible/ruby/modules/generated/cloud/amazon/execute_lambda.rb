  ...
  module Ruby
    module Modules
      ...
      class Execute_lambda < Base
        ...
        # @return [String, nil] If C(tail_log=yes), the result of the task will include the last 4 KB of the CloudWatch log for the function execution. Log tailing only works if you use synchronous invocation C(wait=yes). This is usually used for development or testing Lambdas.
        attribute :tail_log
        validates :tail_log, type: String

        # @return [String, nil] Whether to wait for the function results or not. If I(wait) is C(no), the task will not return any results. To wait for the Lambda function to complete, set C(wait=yes) and the result will be available in the I(output) key.
        attribute :wait
        validates :wait, type: String

        # @return [String, nil] Do not *actually* invoke the function. A C(DryRun) call will check that the caller has permissions to call the function, especially for checking cross-account permissions.
        attribute :dry_run
        validates :dry_run, type: String
