  ...
  module Ruby
    module Modules
      ...
      class Iam_group < Base
        ...
        validates :purge_policy, inclusion: {:in=>[true, false], :message=>"%{value} needs to be true, false"}, allow_nil: true

        # @return [Boolean, nil] Deatach users which not included in users list
        attribute :purge_users
        validates :purge_users, inclusion: {:in=>[true, false], :message=>"%{value} needs to be true, false"}, allow_nil: true
      end
    end
  end
end
