  ...
  module Ruby
    module Modules
      ...
      class Apt < Base
        ...
        # @return [Boolean, nil] If C(yes), remove unused dependency packages for all module states except I(build-dep). It can also be used as the only option.
        attribute :autoremove
        validates :autoremove, inclusion: {:in=>[true, false], :message=>"%{value} needs to be true, false"}, allow_nil: true

        # @return [Boolean, nil] Only upgrade a package if it is already installed.
