  ...
  module Ruby
    module Modules
      ...
      class Yum < Base
        ...
        validates :use_backend, inclusion: {:in=>[:auto, :yum, :yum4, :dnf], :message=>"%{value} needs to be :auto, :yum, :yum4, :dnf"}, allow_nil: true
        ...
        attribute :state
        validates :state, inclusion: {:in=>[:absent, :installed, :latest, :present, :removed], :message=>"%{value} needs to be :absent, :installed, :latest, :present, :removed"}, allow_nil: true
        ...
        # @return [Boolean, nil] If C(yes), removes all "leaf" packages from the system that were originally installed as dependencies of user-installed packages but which are no longer required by any such package. Should be used alone or when state is I(absent),NOTE: This feature requires yum >= 3.4.3 (RHEL/CentOS 7+)
