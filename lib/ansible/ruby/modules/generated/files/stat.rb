  ...
  module Ruby
    module Modules
      ...
      class Stat < Base
        ...
        attribute :get_mime
        validates :get_mime, inclusion: {:in=>[:yes, :no], :message=>"%{value} needs to be :yes, :no"}, allow_nil: true

        # @return [:yes, :no, nil] Get file attributes using lsattr tool if present.
        attribute :get_attributes
        validates :get_attributes, inclusion: {:in=>[:yes, :no], :message=>"%{value} needs to be :yes, :no"}, allow_nil: true
      end
    end
  end
end
