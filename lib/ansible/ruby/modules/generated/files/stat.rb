  ...
  module Ruby
    module Modules
      ...
      class Stat < Base
        ...
        # @return [Boolean, nil] Use file magic and return data about the nature of the file. this uses the 'file' utility found on most Linux/Unix systems.,This will add both `mime_type` and 'charset' fields to the return, if possible.,In 2.3 this option changed from 'mime' to 'get_mime' and the default changed to 'Yes'
        attribute :get_mime
        validates :get_mime, inclusion: {:in=>[true, false], :message=>"%{value} needs to be true, false"}, allow_nil: true

        # @return [Boolean, nil] Get file attributes using lsattr tool if present.
        attribute :get_attributes
        validates :get_attributes, inclusion: {:in=>[true, false], :message=>"%{value} needs to be true, false"}, allow_nil: true
      end
    end
  end
end
