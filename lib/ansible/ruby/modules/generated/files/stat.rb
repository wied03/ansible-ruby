  ...
  module Ruby
    module Modules
      ...
      class Stat < Base
        ...
        validates :checksum_algorithm, inclusion: {:in=>[:md5, :sha1, :sha224, :sha256, :sha384, :sha512], :message=>"%{value} needs to be :md5, :sha1, :sha224, :sha256, :sha384, :sha512"}, allow_nil: true

        # @return [String, nil] Use file magic and return data about the nature of the file. this uses the 'file' utility found on most Linux/Unix systems.,This will add both `mime_type` and 'charset' fields to the return, if possible.,In 2.3 this option changed from 'mime' to 'get_mime' and the default changed to 'Yes'.
        attribute :get_mime
        validates :get_mime, type: String

        # @return [String, nil] Get file attributes using lsattr tool if present.
        attribute :get_attributes
        validates :get_attributes, type: String
      end
    end
  end
end
